...
const SchemaObject = {
  name: Joi.string().trim().regex(/^[A-Za-z][^0-9]+$/).min(3)
    .max(25)
    .required(),
  email: Joi.string().trim().email().regex(/^.+[.]\w{2,3}$/).required(),
  password: Joi.string().trim().min(7).regex(/.{7,}$/).required(),
  confirmPassword: Joi.string().trim().min(7).valid(Joi.ref("password")).required(),
  phoneNumber: Joi.string().trim().regex(/^234[7-9][0-1][0-9]+$/).length(13).required(),
  ...
  amount: Joi.number().positive().min(100).max(10000000)
    .required(),
  type: Joi.string().trim().valid(["savings", "current", "fixed", "fixed deposit", "joint"]).lowercase().required(),
  userType: Joi.string().trim().valid(["admin", "staff"]).lowercase().required(),
